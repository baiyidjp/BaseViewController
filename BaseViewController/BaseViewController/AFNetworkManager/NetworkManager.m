                  ...
                  failure:(failureBlock)failure
{
    ...
    if (httpMethod == HttpMethod_GET) {
        ...
        [self.sessionManager GET:realUrl
                      parameters:params
                        progress:nil
        ...
        [self.sessionManager POST:realUrl
                       parameters:params
                         progress:nil
                          success:successBlock
                          failure:failureBlock];
...
#warning 判断是否登录 djp
    ...
    [self LoginUploadRequestWithMethod:httpMethod
                                   Url:urlStr
                                params:[paramsM copy]
                                 datas:nil
                               success:success
                             ...
                             failure:(failureBlock)failure
{
    ...
    if (datas.count) {
        //调用上传
        [self UploadRequestWithUrl:urlStr
                            params:params
                             datas:datas
                           success:success
                           failure:failure];
        ...
        //调用普通请求
        [self RequestWithMethod:httpMethod
                            Url:urlStr
                         params:params
                        success:success
                        failure:failure];
